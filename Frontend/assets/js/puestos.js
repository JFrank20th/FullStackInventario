...
function GetPuestos() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $("#TablaPuestos tbody").on("click", "a.editar", function () {
                var dataEditar = TablaPuestos.row($(this).parents("tr")).data();
                EditPuesto(dataEditar);

            });

            //Obtener datos al darle click en eliminar
            $("#TablaPuestos tbody").on("click", "a.eliminar", function () {
                var dataEliminar = TablaPuestos.row($(this).parents("tr")).data();
                desactivarPuesto(dataEliminar);

            });

            //Funcion desactivar
            function desactivarPuesto(dataE) {
                if (dataE.Id_Estado == "Activo") {
                    swal({
                        title: "¿Está seguro de Desactivar el Puesto?",
                        text: "" + dataE.Id_Puesto + "",
                        icon: "warning",
                        buttons: true,
                        dangerMode: true,
                    })
                        .then((willDelete) => {
                            if (willDelete) {
                                ...
                                fetch(Url_Backend + "ActualizarPuesto", requestOptions).then(response => response.text(
                                )).then(result => {
                                    var resultado = result;
                                    if (resultado != "false") {
                                        swal("Upps!", "No pudo desactivarse el Puesto", "error");
                                    } else if (resultado == "false") {
                                        swal("Ey!", "El Puesto seleccionada se ha desactivado con éxito", "success").then((result) => {
                                            GetPuestos();
                    ...
                    swal({
                        title: "¿Está seguro de Activar el Puesto?",
                        text: "" + dataE.Id_Puesto + "",
                        icon: "warning",
                        buttons: true,
                        dangerMode: true,
                        ...
                        .then((willDelete) => {
                            if (willDelete) {
                                ...
                                fetch(Url_Backend + "ActualizarPuesto", requestOptions)
                                    .then(response => response.text(
                                    ))
                                    .then(result => {
                                        var resultado = result;
                                        if (resultado != "false") {
                                            swal("Upps!", "No pudo activarse el Puesto", "error");
                                        } else if (resultado == "false") {
                                            swal("Ey!", "El Puesto seleccionado se ha activado con éxito", "success").then((result) => {
                                                GetPuestos();
                                                /*if (result.value === true) {
                                                    window.location.href = 'Puestos.html'
                                                }*/
            ...
            function EditPuesto(data) {
                ...
                document.getElementById("IdPuestoActu").value = data.Id_Puesto;
...
function GuardarPuesto() {
    ObtenerToken(User.Identificacion_Usuario, User.Password);
    var puestoAgre = $("#puestoAgre").val();
    var SedeAgre = $("#SedeAgre").val();
    var PisoAgre = $("#PisoAgre").val();

    if (SedeAgre == "" || PisoAgre == "" || puestoAgre == "") {
        swal("Ey!", "Es obligatorio llenar los campos", "error");
        return false;
    }

    var Param = {
        Id_Puesto: puestoAgre,
        Id_Sede: SedeAgre,
        Id_Piso: PisoAgre,
        Id_Estado: '1'
    };
    $.ajax({
        url: Url_Backend + "GuardarPuesto",
        type: "POST",
        dataType: "json",
        contentType: "application/x-www-form-urlencoded; charset=utf-8",
        headers: {
        ...
        beforeSend: function () { },
        success: function (response) {
            swal("Bien!", "El Puesto fue agregado correctamente", "success").then((result) => {
                GetPuestos();
                /*if (result.value == true) {
                    window.location.href = 'Puestos.html'
                }*/
        ...
        error: function (err) {
            swal("Ey!", "Error al agregar el Puesto: " + err, "error");
        },
        timeout: 300000,
    });
};
...
$("#ActualizarPuesto").click(function () {
    ActualizarPuesto();
});
...
function ActualizarPuesto() {
    debugger;
    ObtenerToken(User.Identificacion_Usuario, User.Password);
    var IdPuestoActu = $("#IdPuestoActu").val();
    var SedeActu = $("#SedeActu").val();
    var PisoActu = $("#PisoActu").val();
    var EstadoActu = $("#EstadoActu").val();

    if (SedeActu == "" ||  PisoActu == "") {
        swal("Ey!", "Es obligatorio llenar los campos", "error");
        return false;
    }
    var Param = {
        Id_Puesto: IdPuestoActu,
        Id_Sede: SedeActu,
        Id_Piso: PisoActu,
        Id_Estado: EstadoActu
    };
    $.ajax({
        url: Url_Backend + "ActualizarPuesto",
        type: "POST",
        dataType: "json",
        contentType: "application/x-www-form-urlencoded; charset=utf-8",
        headers: {
        ...
        beforeSend: function () { },
        success: function (response) {
            swal("Bien!", "El Puesto  fue actualizada correctamente", "success").then((result) => {
                GetPuestos();
                /*if (result.value == true) {
                    window.location.href = 'Puestos.html'
                }*/
        ...
        error: function (err) {
            swal("Ey!", "Error al actualizar El Puesto: " + err, "error");
        },
        timeout: 300000,
    });
};
